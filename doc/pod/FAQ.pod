...
#
# $Id: FAQ.pod,v 4.0 2004/11/16 20:57:31 matt Exp $
#

=head1 What features does the Toaster support?
  ...
  Builds self-signed SSL certs for Apache (HTTPS), 
    Qmail (SMTP TLS), & Courier (IMAPS/POP3S)


=head1 What programs will be installed?
...
All of the following *can* be installed but you have control via toaster-watcher.conf over many of the optional installs. You also have control over many of the popular options for building certain programs like MySQL, Apache, Vpopmail, etc.
  ...
  Qmail        : Secure, fast, and reliable MTA
  ...
  qmailadmin   : Vpopmail domain administration
  vqadmin      : Vpopmai system admin panel
  ...
  razor        : Spam signatures database
  pyzor        : Python spam signatures database
  dcc          : Spam fighting tool
  procmail     : mail delivery agent
  maildrop     : Procmail like mail delivery agent
  socklog      : pipe qmail logs to another host
  cronolog     : automatic log rotater
  qmailanalog  : useful for summarizing qmail logs
  isoqlog      : qmail log processor, pretty HTML
  gnupg        : GNU version of PGP
...
=head1 What are the mail retrieval options?
...
=head1 How do I manage mail options for a domain?

Point a web broswer at https://mail.example.com/cgi-bin/qmailadmin


=head1 Where do I find the maildomain script?
...
=head1 How do I convert Multilog date stamps to human readable date stamps?

  tai64nlocal < logfile > qmaillog.tmp


=head1 How do I find statistics about my mail queue:
...
=head1 How do I enable the sqwebmail calendar?

echo "local" &gt; /usr/local/share/sqwebmail/calendarmode


=head1 When will you add support for my platform?
...
=head1 How do I block connections from servers with no reverse DNS?
...
=head1 How do I use SpamAssassin & ClamAV?

See the Filtering section at http://www.tnpi.biz/internet/mail/toaster/filter.shtml


=head1 Whats the best way to view/expire messages from the queue?

Use qqtool: http://www.tnpi.biz/internet/mail/qqtool/


=head1 Why are IMAP connections getting reset?
...
=head1 How do I delete old messages in the Spam folders?
...
=head1 Where is the UCSPI-TCP patch?
...
=head1 How can I modify the Sqwebmail interface?

The html temples live in /usr/local/share/sqwebmail/html/


=head1 How can I modify the Qmailadmin interface?

The html templates live in /usr/local/share/qmailadmin/html/


=head1 What httpd.conf changes are needed?

See contrib/httpd.conf-2.0.diff in the Mail::Toaster distribution.


=head1 Can you explain the Anti-Spam options please?
...
=head1 Why does SpamAssassin take so long to check messages? 
...
=head1 How do I set up per-user SpamAssassin preferences?
...
=head1 What needs to be in /etc/rc.conf?
...
=head1 How do I get QmailScanner Statistics to work?
...
=head1 RBL's work, but I need email from a RBL'ed server.

You have a couple options: 

  1. Remove the offending RBL from toaster-watcher.conf

  2. set RBLSMTPD="" for the IP(s) in tcp.smtp

The latter is the better approach.


=head1 How do I sent test spam to myself remotely?

Grab the spam, including all headers, and paste it into a telnet session 
as described on Matt's "mail forge" page. 

http://www.tnpi.biz/internet/mail/forge.shtml

   % telnet mail.domain.com smtp
   Trying 64.224.19.12...
...
=head1 What is the MySQL query command for setting up Vpopmail?
...
=head1 Help, I'm having problems installing FreeBSD on my Kitchenaid toaster
...
=head1 How do I upgrade my hard drive? 
...
=head1 How do I migrate my Mail::Toaster from one server to another.
...
=head1 How do users access their Spam folder?
...
=head1 I enabled spam filtering in Qmailadmin. Where is the Spam folder?


It get's automatically created when the first spam message arrives. It is automatically subscribed to your IMAP folders.


=head1 How do I route outgoing mail through my ISP's SMTP server?
